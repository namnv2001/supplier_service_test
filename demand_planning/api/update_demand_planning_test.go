...
type updateDemandPlanningSuite struct {
	tests.TestSuite
	faker *faker.Faker
	*gorm.DB
	fixedTime time.Time
...
type updateDemandPlanningObj struct {
...
func TestUpsertDemandPlanning(t *testing.T) {
	ts := &updateDemandPlanningSuite{}
...
func (ts *updateDemandPlanningSuite) tearDown(db *gorm.DB) {
...
func (ts *updateDemandPlanningSuite) assert(req *api.UpsertDemandPlanningRequest, wantFile string) {
	defer func() {
		ts.tearDown(ts.DB)
	}()
	res, err := ts.DemandPlanningClient.UpsertDemandPlanning(ts.Context, req)
	g := goldie.New(ts.T())
	prefix := "update_demand_planning"
	if err != nil {
		assert.Nil(ts.T(), res)
	...
	var wantData updateDemandPlanningObj
	res.TraceId = ""
	wantData.Resp = res
...
func (ts *updateDemandPlanningSuite) Test200_happyCase_siteSkuDemand() {
...
func (ts *updateDemandPlanningSuite) Test400_siteSkuDemand_update_time_in_past() {
...
func (ts *updateDemandPlanningSuite) Test200_happyCase_siteGroupDemand() {
...
func (ts *updateDemandPlanningSuite) Test400_siteGroupDemand_update_time_in_past() {
